...
- [packs](https://packs.sh) by [Tunajam](https://github.com/tunajam) - Homebrew-style package manager for AI agent skills. Browse 30+ reusable skills in a TUI (`packs`), install with one command (`packs get commit-message`), or share your own (`packs submit @user/skill`). Auto-detects Claude Code, Codex, and other agents for smart installation paths.
