...
- [Plan-Build-Run](https://github.com/SienkLogic/plan-build-run) by [SienkLogic](https://github.com/SienkLogic) - Context-engineered development workflow that solves context rot through subagent delegation. Keeps your orchestrator under ~15% context by delegating heavy work to fresh 200k-token agent windows. 21 slash commands, 10 specialized agents, 15 lifecycle hooks, wave-based parallel execution, goal-backward verification, and kill-safe file-based state. Works on every Claude Code tier.
