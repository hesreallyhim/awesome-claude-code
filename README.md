...
- [Kontour Travel Planner](https://github.com/Bookingdesk-AI/kontour-travel-planner) by [Bookingdesk AI](https://github.com/Bookingdesk-AI) - AI travel planning skill using a 9-dimension progressive planning model. Stage-based conversation flow (Discover → Develop → Refine → Confirm) with guided discovery, conflict detection, and confidence scoring. Includes reference data for 200+ destinations, 500 airports, airlines, and budget benchmarks — no API needed for basic planning intelligence. Produces structured itineraries with Google Maps export.
