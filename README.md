...
- [Auto-claude-code-research-in-sleep](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep) by [wanshuiyin](https://github.com/wanshuiyin) - 8 skills for autonomous ML research: Claude Code executes (reads papers, writes code, runs GPU experiments) while an external LLM reviews via Codex MCP (scores, finds weaknesses, suggests fixes). The auto-review-loop skill iterates review→fix→re-review overnight — a real run improved a paper from 5/10 to 7.5/10 across 4 rounds with 20+ GPU experiments, zero human intervention.
