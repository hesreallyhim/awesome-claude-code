...
- [TRINITY](https://github.com/rsv413-EG/trinity) by [grouwels](https://github.com/grouwels) — A cognitive AI operating system with seven specialized agents running on your own hardware. TRINITY is ARCHITECT (orchestrator), CORE (developer), FAULT (adversarial inspector), SCOUT (watcher), KEEPER (memory), FORGE (tools), and HERALD (outbound comms). Full memory vault, daily logs, code review gate, Ralph Loop iterative builds, and autonomous Sentry bug-fix pipeline. Built on OpenClaw and runs on a Raspberry Pi + local GPU. This is what it looks like when someone stops using Claude as a tool and starts treating it as a team.
