...
- [session-graph](https://github.com/robertoshimizu/session-graph) by [Roberto Shimizu](https://github.com/robertoshimizu) - Extracts (subject, predicate, object) knowledge triples from session logs using an LLM, links entities to Wikidata, and loads into a SPARQL triplestore. One SPARQL query replaces grepping through JSONL files. Supports Claude Code, DeepSeek, Grok, and Warp sessions. 138K triples from 52 sessions at ~$0.70.
