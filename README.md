...
- [claude-synapse](https://github.com/PooterAi/claude-synapse) by [PooterAi](https://github.com/PooterAi) - Cognitive memory plugin that gives Claude Code persistent, intelligent memory across sessions. 18 MCP tools including brain_store, brain_recall, brain_health, and brain_prune. Features a quality gate (deduplication, fingerprinting, conflict detection), 9-stage hippocampal retrieval pipeline, learned rules extraction from recurring patterns, and a two-strike error circuit breaker. Zero LLM cost -- pure computation with fully local JSON file storage. Includes 5 lifecycle hooks for session continuity.
