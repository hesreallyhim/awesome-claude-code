...
- [Autonomous Coding Toolkit](https://github.com/parthalon025/autonomous-coding-toolkit) by [parthalon025](https://github.com/parthalon025) - Extends the Superpowers skill chain with headless batch execution, quality gates between every batch (lesson check + ast-grep + tests + memory), 79 community lessons across 6 failure clusters, Multi-Armed Bandit routing via Thompson Sampling, and fresh 200k context per batch to prevent context degradation on long tasks. Includes 7 specialized agents, a research phase, and a positive policy system. 50 tests, fully autonomous `run-plan.sh` for unattended execution.
