...
- [Washin Claude Skills](https://github.com/sstklen/washin-claude-skills) by [sstklen](https://github.com/sstklen) - 112 battle-tested Claude Code skills extracted from 200+ production bugs building an API platform at an animal sanctuary in Japan. Covers Docker/DevOps (OOM, ghost containers, WAL traps), API architecture (multi-provider gateway, quota enforcement), billing patterns (phantom refunds, rounding traps), Hono/Bun gotchas, and security audit methodology. One-command install with category filtering.
