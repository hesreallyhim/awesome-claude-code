...
- [Skillfold](https://github.com/byronxlg/skillfold) by [Byron Smith](https://github.com/byronxlg) - YAML-based pipeline compiler for multi-agent AI teams. Define atomic skills, compose them into agents, wire team flows with typed state, and generate orchestrators - all from a single config file. Outputs standard SKILL.md files. Features an `adopt` command for existing Claude Code setups, a shared library of 11 reusable skills, and multi-platform support (Claude Code, Cursor, Codex, Gemini CLI).
- [sudocode](https://github.com/sudocode-ai/sudocode) by [ssh-randy](https://github.com/ssh-randy) - Lightweight agent orchestration dev tool that lives in your repo. Integrates with various specification frameworks. It's giving Jira.
