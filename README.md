...
- [Syntropic](https://github.com/petercholford-ship-it/syntropic-cli) by [Peter Cholford](https://github.com/petercholford-ship-it) - An npm CLI (`npx syntropic init`) that scaffolds a complete development methodology into AI coding tool instruction files. Creates CLAUDE.md with 9 Evergreen Rules (research-before-coding, hypothesis-driven debugging, pre-flight checks, and more), 6 generic subagents, and a SKILL.md for automatic skill discovery. Also generates configs for Cursor, Windsurf, Copilot, and Codex. Zero dependencies, appends to existing files without overwriting.
