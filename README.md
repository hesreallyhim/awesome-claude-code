...
- [claude-code-hooks](https://github.com/Payshak/claude-code-hooks) by [Payshak](https://github.com/Payshak) - The npm-native TypeScript SDK for Claude Code hooks. `defineHook()` gives you fully typed payloads for all five hook events, response builders (`allow()`, `block()`, `inject()`), glob and regex matchers, and a `.handle()` method for unit testing hooks without touching stdin/stdout. Zero dependencies — the typed foundation you build hooks on top of, not just another script collection.
